...
async def report(
    ...
    if output_dir != None:
        ...
        out_plot_keystroke_timeline = str(
            "./"
            + output_dir
            + "/"
            + "packet-strider-ssh "
    ...
    print(
        "\n\u250F\u2501\u2501\u2501\u2501 Reporting results for stream {}".format(
            stream
        )
    )
    ...
    print(
        "\u2503 {} packets in total, first at {}".format(
            num_packets, time_first_packet_gmt
        )
    )
    print(
        "\u2503 \033[0;36;40m{}:{}\033[0m -> \033[0;31;40m {}:{}\033[0m".format(
            sip, sport, dip, dport
        )
    )
    ...
    if (do_direction == "forward" or do_direction == "both") and not meta_only:
        if num_f_init_events > 0:
            print(
                "\u2503       \033[0;36;40m {} Forward SSH login/init events\033[0m".format(
                    num_f_init_events
                )
            )
        if num_f_keystroke_events > 0:
            print(
                "\u2503       \033[0;36;40m {} Forward keystroke related events\033[0m".format(
                    num_f_keystroke_events
                )
            )
    ...
    if (do_direction == "reverse" or do_direction == "both") and not meta_only:
        if num_r_init_events > 0:
            print(
                "\u2503       \033[1;31;40m {} Reverse SSH login/init events\033[0m".format(
                    num_r_init_events
                )
            )
        if num_r_keystroke_events > 0:
            print(
                "\u2503       \033[1;31;40m {} Reverse keystroke related events\033[0m".format(
                    num_r_keystroke_events
                )
            )
        # TODO fix this, calculate aggregate and report on this rather than separate techniques.
    ...
    if window_matrix and not meta_only and do_plots:
        with concurrent.futures.ProcessPoolExecutor() as pool:
            ...
            if len(matrix) > 10:
                print(
                    "\u2503 Plotting packets {}-{} size histogram to '{}'".format(
                        zleft, zright, out_plot_predictions
                    )
                )
            ...
            print(
                "\u2503 Plotting packets {}-{} Data Movement predictions to '{}'".format(
                    zleft, zright, out_plot_predictions
                )
            )
            ...
            if results:
                print(
                    "\u2503 Plotting packets {}-{} keystroke timeline to '{}'".format(
                        zleft, zright, out_plot_keystroke_timeline
                    )
                )
...
def pretty_print(results):
    ...
    for result in results:
        ...
        if result[0] == "forward":
            print(
                "\u2503       \033[1;36;40m{:<10}{:<10}{:<10}{:<10}{:^10}{:^10}{:^10}\033[0m".format(
                    result[3],
                    round(result[6], 3),
                    round(result[7], 3),
                    result[0],
            ...
            print(
                "\u2503       \033[1;31;40m{:<10}{:<10}{:<10}{:<10}{:^10}{:^10}{:^10}\033[0m".format(
                    result[3],
                    round(result[6], 3),
                    round(result[7], 3),
                    result[0],
...
def plot_window_stat_predictions(
    stream, window_matrix, window, stride, file, out_plot_predictions, zleft, zright
):
    """Plots the window analysis including the exfil prediction
       which does not rely of stream meta being known"""
    df_stream = pandas.DataFrame(
        window_matrix,
        columns=[
            "Packet number (datum)",
            "stream",
            "window",
            "stride",
            "Client packet size (normalized)",
            "client_packets",
            "client_packets_size",
            "Client size variance",
            "Server packet size (normalized)",
            "server_packets",
            "server_packets_size",
            "Server Size variance",
            "Client:Server packet ratio",
            "Client:Server size ratio",
            "Window Exfiltration prediction",
            "Window Infiltration prediction",
            "Time elapsed normalized",
            "Packet Time delta normalized",
            "Datum packet size",
            "Is forward enter child",
            "Is reverse enter child",
            "Infiltration prediction aggregate",
            "Exfiltration prediction aggregate",
        ],
    )
    ...
    title = (
        "Strider - protocol:SSH"
        + "\n"
        + "Data Movement Predictions for pcap '"
        + file
        + "'\n"
        + "Stream"
        + str(stream)
...
def plot_packet_size_histogram(stream, matrix, output_dir, base_file, zleft, zright):
    """Plots the packet size histogram
       which does not rely of stream meta being known"""
    out_plot_packet_size_histogram = str(
        "./"
        + output_dir
        + "/"
        + "packet-strider-ssh "
...
def plot_keystroke_timeline(
    ...
    for result in results:
        ...
        f_login_success = (
            f_login_failure
        ) = f_keystroke = f_delete = f_exfil = f_up_down_paste = 0
    ...
    df_indicators = pandas.DataFrame(
        ...
        columns=[
            ...
            "REVERSE login prompt",
            "REVERSE login success",
            "REVERSE login failure",
            "REVERSE keystroke",
            "REVERSE delete",
            "REVERSE Enter",
            "Reverse UP/DOWN/Paste",
            "Bytes exfiled when Reverse Enter key pressed",
            "Forward key offer",
            "Forward key accept",
            "Forward login prompt",
            "Forward login success",
            "Forward login failure",
            "Forward keystroke",
            "Forward delete",
            "Forward Enter key",
    ...
    df_stream_merged_results = df_stream.merge(
        df_indicators, how="outer", left_on="Packet number (datum)", right_on="index"
    ).fillna(0)

    df_stream_merged_results = df_stream_merged_results.loc[zleft:zright]

    title = (
        "Strider - protocol:SSH Keystroke predictions timeline"
        + "\n"
        + file
        + "    Stream "
        + str(stream)
...
def zooms(zoom, num_packets):
    ...
    if "-" in zoom:
        ...
        if (
            ...
            or zleft < 0
            or zright < 0
            or zleft > num_packets
            or zright > num_packets
        ):
            print(
                "... ignoring out of bounds packet zooms, max zoom out is -z 0-{}".format(
                    num_packets
                )
            )
            zleft = 0
        ...
        print(
            "... ignoring invalid packet zooms, the max zoom out is -z 0-{}".format(
                num_packets
            )
        )
        zleft = 0
