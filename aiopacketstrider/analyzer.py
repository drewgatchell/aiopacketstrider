...
from aiopacketstrider.metadata import MetaSize
...
async def analyze(
    ...
    if meta_size[1] != 0:
        if do_direction == "forward":
            ...
            results_f_logins, fwd_logged_in_at_packet = await loop.run_in_executor(
                None, scan_for_forward_login_attempts, matrix, meta_size, pcap
            )
            print("   ... Scanning for Forward key accepts")
            results_f_key_accepts = await loop.run_in_executor(
                None, scan_for_forward_host_key_accepts, pcap, fwd_logged_in_at_packet
            )
            print("   ... Scanning for Forward login prompts")
            results_f_login_prompts = await loop.run_in_executor(
                None,
            ...
            if keystrokes and fwd_logged_in_at_packet > 0:
                ...
                results_f_keystroke = await loop.run_in_executor(
                    None,
                    scan_for_forward_keystrokes,
                    matrix,
                    meta_size,
                    pcap,
                    fwd_logged_in_at_packet,
            ...
            results_r_init, reverse_init_start = await loop.run_in_executor(
                None, scan_for_reverse_session_initiation, matrix, meta_size, pcap
            )
            if reverse_init_start != 0:
                print("   ... Scanning for Reverse Session logins")
                results_r_logins = await loop.run_in_executor(
                    None,
                    scan_for_reverse_login_attempts,
                    matrix,
                    meta_size,
                    pcap,
                    reverse_init_start,
                ...
                if keystrokes:
                    ...
                    results_r_keystroke = await loop.run_in_executor(
                        None,
                        scan_for_reverse_keystrokes,
                        matrix,
                        meta_size,
                        pcap,
                        reverse_init_start,
    ...
    if do_windowing_and_plots:
        window_matrix = await matrix_builder.construct_window_matrix(
...
def order_keystrokes(matrix_unordered: List[int], meta_size: MetaSize) -> List[int]:
    ...
    for i in range(0, len(pcap) - 4):
        ...
        if "message_code" in dir(pcap[i].ssh):
            ...
            if int(pcap[i].ssh.message_code) == 21 and "message_code" not in dir(
                ...
                results_f_key_accepts = results_f_key_accepts + [
                    [
                        "forward",
                        "key accepted ",
                        i,
...
def scan_for_forward_login_prompts(matrix, meta_size, pcap, fwd_logged_in_at_packet):
    ...
    for i in range(0, min(len(matrix), stop_at)):
        if matrix[i] == size_login_prompt:
            relative_timestamp = float(pcap[i].sniff_timestamp) - timestamp_first
            results_f_login_prompts = results_f_login_prompts + [
                [
                    "forward",
                    "login prompt ",
                    i,
...
def scan_for_forward_login_attempts(matrix, meta_size, pcap):
    ...
    for i in range(8, (min(len(matrix) - 2, 300))):
        ...
        if (
            matrix[i] == size_login_prompt
            and matrix[i + 1] > 0
            and matrix[i + 2] == size_login_prompt
        ):
            relative_timestamp = float(pcap[i + 1].sniff_timestamp) - timestamp_first
            results_f_logins = results_f_logins + [
                [
                    "forward",
                    "login failure",
                    i,
...
def scan_for_reverse_login_attempts(matrix, meta_size, pcap, reverse_init_start):
    ...
    for i in range(reverse_init_start, min((len(matrix) - 4), 300)):
        if (
            matrix[i] == size_reverse_login_prompt
            and matrix[i + 1] < -size_reverse_login_prompt
            and matrix[i + 2] > size_reverse_login_prompt
            and matrix[i + 3] < -size_reverse_login_prompt
            and matrix[i + 4] == size_reverse_login_prompt
        ):
            relative_timestamp = float(pcap[i].sniff_timestamp) - timestamp_first
            results_r_logins = results_r_logins + [
                [
                    "reverse",
                    "login prompt ",
                    i,
        ...
        if (
            matrix[i] == size_reverse_login_prompt
            and matrix[i + 1] < -size_reverse_login_prompt
            and 0 < matrix[i + 2] < size_reverse_login_prompt
            and matrix[i + 3] < -size_reverse_login_prompt
            and 0 < matrix[i + 4] < size_reverse_login_prompt
        ):
...
def scan_for_forward_keystrokes(matrix, meta_size, pcap, fwd_logged_in_at_packet):
    ...
    while i < len(matrix) - 2:
        ...
        if size_this == forward_keystroke_size:
            ...
            if (
                size_next == -forward_keystroke_size
                and size_next_next == forward_keystroke_size
            ):
                relative_timestamp = float(pcap[i].sniff_timestamp) - timestamp_first
                keystrokes = keystrokes + 1
                results_f_keystroke = results_f_keystroke + [
                    [
                        "forward",
                        "keystroke    ",
                        i,
                ...
                size_next == -(forward_keystroke_size + 8)
                and size_next_next == forward_keystroke_size
            ):
                relative_timestamp = float(pcap[i].sniff_timestamp) - timestamp_first
                keystrokes = keystrokes + 1
...
def scan_for_reverse_session_initiation(matrix, meta_size, pcap):
    ...
    for i in range(0, len(matrix) - 3):
        if (
            matrix[i + 1] == -(size_newkeys_next + 40)
            and matrix[i + 2] == -(size_newkeys_next2 - 40)
            and matrix[i + 3] < 0
            and abs(matrix[i + 3]) >= (matrix[i + 2])
        ):
            relative_timestamp = float(pcap[i + 1].sniff_timestamp) - timestamp_first
            reverse_init_start = i
            finish = i + 3
            results_r_init = results_r_init + [
                [
                    "reverse",
                    "session init ",
                    reverse_init_start,
...
def scan_for_reverse_keystrokes(matrix, meta_size, pcap, reverse_init_start):
    ...
    while i < len(matrix) - 2:
        ...
        if size_this == reverse_keystroke_size:
            ...
            if (
                size_next == -reverse_keystroke_size
                and size_next_next == reverse_keystroke_size
            ):
                keystrokes = keystrokes + 1
                relative_timestamp = float(pcap[i].sniff_timestamp) - timestamp_first
                results_r_keystroke = results_r_keystroke + [
                    [
                        "reverse",
                        "keystroke    ",
                        i,
            ...
            # debug changed +8 to -8
                ...
                size_next == -(reverse_keystroke_size - 8)
                and size_next_next == reverse_keystroke_size
            ):
            ...
            # debug changed +8 to -8
                ...
                size_next == -reverse_keystroke_size
                and size_next_next > -(reverse_keystroke_size - 8)
                and keystrokes > 0
            ):
                relative_timestamp = float(pcap[i].sniff_timestamp) - timestamp_first
                i_enterkey_pressed = i
